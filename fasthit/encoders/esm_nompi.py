...
class ESMNOMPI(fasthit.Encoder):
    def __init__(
        self,
        encoding: str,
        alphabet: str,
    ...
    def _embed_msa(
        ...
        def _write_msa(seq, name, **kwargs):
            ...
            _=subprocess.run(
                [
                    ...
                    "-d", kwargs["database"], "-v", "1", "-maxres", "50000", "-maxres", "50001", "-cpu", str(
                        kwargs["n_threads"]),
        ...
        def _get_msa(name, **kwargs):
            ###
            _hamming_distance = lambda s1, s2: sum(ch1 != ch2 for ch1, ch2 in zip(s1, s2))
            ###
        ...
        with Parallel(n_jobs=self._msa_batch_size) as parallel:
            for seqs_batch, names_batch in zip(
                ...
                parallel(
                    delayed(_write_msa)(seq, name, **kwargs) for seq, name in zip(seqs_batch, names_batch)
                )
