...
class SampleCollector(ISerialCollector):
    ...
    def __init__(
        ...
        assert env._cfg.auto_reset == False, env._cfg.auto_reset
        ...
        while collected_sample < n_sample:
            ...
            for env_id, timestep in timesteps.items():
                ...
                if timestep.done:
                    ...
                    ###
                    starting_seq = self._reset_param(return_data)
                    reset_param = {"starting_seq": starting_seq}
                    self._env.reset({env_id: reset_param})
                    ###
    ...
    def _reset_param(self, data: list) -> str:
        obs = {
            s_utils.one_hot_to_string(
                step["obs"]["sequence"].cpu().numpy(), self._alphabet
            ) : step["obs"]["fitness"].cpu().numpy() for step in data
        }
        top_fitness = max(obs.values())
        top_sequences = [
            seq for seq, fitness in obs.items() if fitness >= 0.9 * top_fitness
        ]
        if len(top_sequences) > 0:
            starting_seq = np.random.choice(top_sequences)
        else:
            starting_seq = np.random.choice(
                [seq for seq, _ in obs.items()]
            )
        return starting_seq
