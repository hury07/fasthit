...
class BLOSUMKernelRsample(Kernel):
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, diag: bool, **params):
        ...
        Kstar = torch.ones([self.N_, self.N_], dtype=torch.float32)

        for p in range(self.L):
            x1_preceed = np.expand_dims(self.x1[:, p], 0)
            x2_preceed = np.expand_dims(self.x2[:, p], 0)
            k_index = torch.meshgrid(x1_preceed, x2_preceed).reshape(2, -1)
            k_p = self.project_B[k_index[0, :], k_index[1, :]].reshape(N, self.M)
            Kstar = Kstar * k_p
